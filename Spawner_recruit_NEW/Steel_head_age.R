...
fn <- function(par,DPSDPE,years,Percent4,Percent5,Percent6,Repeat6,data){
    ...
    nll <--1*sum(log(dnorm(eps$ages$SpawnerAdult[8:(years-1)],eps$data$FosterCount[8:(years-1)],par[2])))
...
fninit <- function(par,DPSDPE,years,Percent4,Percent5,Repeat5,Repeat6,data){
    ...
    eps2 <-log(eps$data$FosterCount[8:(years-1)])-log(eps$ages$SpawnerAdult[8:(years-1)])
    nll <--1*sum(log(dnorm((eps2),0,par[3])))
return(nll)
}
...
RS <- exp(mean(log(data$OceanAdults[8:(8+5)]/ages$SpawnerAdult[(8):(8+5)])))
...
for( i in 1:ssize)
{
    ...
    RSmat[i,(8):(years-1)] <- out$data$OceanAdults[(8):(years-1)]/out$ages$SpawnerAdult[(8):(years-1)]
    QETmat[i,] <- ifelse(out$QET[1:(years)]>100,0,1)
    RSvec[i]=out$RS
    Abunvec[i]=out$abund
    marineSvec[i] <- mean(out$sim[8:(8+5)])
    QETvec[i] <- ifelse(min(out$QET[8:(years-1)],na.rm=TRUE)>100,0,1)
    confi2[i] <- out$confi
...
for( i in 1:ssize)
{
    ...
    RSmat[i,(8):(years-1)] <- (out$data$OceanAdults[(8):(years-1)]+out2$data$OceanAdults[(8):(years-1)])/(out$ages$SpawnerAdult[(8-4):(years-5)]+out2$ages$SpawnerAdult[(8-4):(years-5)])
    QETmat[i,] <- ifelse(out$QET[1:years]+out$QET[1:years]>100,0,1)
    RSvec[i]=exp(mean(log((out$data$OceanAdults[(8):(8+5)]+out2$data$OceanAdults[(8):(8+5)])/(out$ages$SpawnerAdult[(8):(8+5)]+out2$ages$SpawnerAdult[(8):(8+5)]))))
    Abunvec[i]=exp(mean(log((out$ages$SpawnerAdult[(21):(30)]+out2$ages$SpawnerAdult[21:30]))))
    marineSvec[i] <- mean((out$sim[8:(8+5)]+out2$sim[8:(8+5)])/2)
    QETvec[i] <- ifelse(min(out$QET[8:(years-1)]+out2$QET[8:(years-1)],na.rm=TRUE)>100,0,1)
