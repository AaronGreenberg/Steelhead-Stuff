...
plot_magic <- function(out,sdcon,z)
{
    ...
    if(length(sdcon)==3){
        ...
        for( i in 1:1000)
        {
            out <- modelinit(confi[i],confi2[i],DPSDPE,years,Percent4,Percent5,Percent6,Repeat6,data)
            conmat[i,] <- out$ages$SpawnerAdult
        }
    }
    ...
    plot(data$FosterCount[7:(years-1)],out$ages$SpawnerAdult[7:(years-1)],ylim=c(0,400),type="o",xlab="Foster Count Observed", ylab="Foster Count Predicted")
    linm=lm(out$ages$SpawnerAdult[7:(years-1)]~data$FosterCount[7:(years-1)])
    abline(linm,col="red")
    print(summary(linm))
